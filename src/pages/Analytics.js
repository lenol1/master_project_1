...
function Analytics() {
    ...
    function exportCSV() {
        ...
        const rows = filteredTx.map(tx => [tx.category, tx.description, tx.date ? tx.date.slice(0, 10) : '', tx.amount, tx.note || '-']);
        let csv = header.join(',') + '\n';
    ...
    return (
        <div className="analyticsPage">
            ...
            <div className="analyticsKPI">
                <div className="kpiCard">{t('analytics.totalIncome', 'Total Income')}: ${totalIncome}</div>
                <div className="kpiCard">{t('analytics.totalExpense', 'Total Expense')}: ${totalExpense}</div>
                <div className="kpiCard">{t('analytics.balance', 'Balance')}: ${balance}</div>
            ...
            {/* Експорт */}
            <div className="analyticsExport">
                <button onClick={exportCSV}>{t('analytics.exportCSV', 'Export CSV')}</button>
                <button onClick={exportPDF}>{t('analytics.exportPDF', 'Export PDF')}</button>
            </div>
        </div>
    );
}
